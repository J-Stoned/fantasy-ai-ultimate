...
-- Version 4: Handle existing tables and type mismatches
...
-- First check if player_game_logs exists with wrong types
DO $$
BEGIN
  -- Check if player_game_logs exists
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'player_game_logs') THEN
    -- Check the data type of player_id column
    IF EXISTS (
      ...
      -- Drop the table if it has UUID type
...
-- 9. Drop and recreate view to ensure correct types
DROP VIEW IF EXISTS v_player_all_stats;
...
SELECT 
  ...
  'game_logs' as source
FROM players p
JOIN player_game_logs pgl ON p.id::INTEGER = pgl.player_id
CROSS JOIN LATERAL jsonb_each(pgl.stats);
...
-- 11. Create search function for players (with explicit casts)
...
BEGIN
  ...
  SELECT 
    ...
    p.position[1] AS player_position,
...
-- Summary: Version 4 improvements:
-- 1. Handles existing tables with wrong types
-- 2. Explicit type casts in views and functions
-- 3. Drops and recreates tables if they have UUID types
-- 4. Ensures all foreign keys are INTEGER type
-- 5. Compatible with existing integer-based schema