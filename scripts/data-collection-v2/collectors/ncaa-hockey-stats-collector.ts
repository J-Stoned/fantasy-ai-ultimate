#!/usr/bin/env tsx
/**
 * 🏒 NCAA HOCKEY STATS COLLECTOR - ULTRA OPTIMIZED
 * 
 * Collects detailed stats for all 2,321 NCAA Hockey games
 * Similar to NHL but with college-specific considerations
 */
...
const API_CONCURRENCY = 150; // 150 concurrent API calls for NCAA Hockey
const DB_BATCH_SIZE = 1500; // Insert 1500 stats at once

export class NCAAHockeyStatsCollector {
  ...
  constructor() {
    ...
    console.log(chalk.cyan.bold('\n🏒 NCAA HOCKEY STATS COLLECTOR - 2,321 GAMES\n'));
  ...
  async collect() {
    try {
      // Cache all NCAA Hockey players first
      await this.cacheNCAAHockeyPlayers();
      
      // Get all NCAA Hockey games needing stats
      ...
      console.log(chalk.cyan(`Found ${this.totalGames} NCAA Hockey games needing stats\n`));
      ...
      console.error(chalk.red('❌ NCAA Hockey stats collection failed:'), error);
    } finally {
      // Don't end pool - let master collector handle it
    }
  }
  
  private async cacheNCAAHockeyPlayers() {
    console.log(chalk.cyan('📦 Caching NCAA Hockey players...'));
    
    const players = await pgPool.query(`
      SELECT id, our_player_id, espn_id, name
      FROM players_master
      WHERE sport = 'NCAA_HOCKEY'
    `);
    ...
    console.log(chalk.green(`✅ Cached ${players.rows.length} NCAA Hockey players\n`));
  }
  ...
  private async collectGameStats(game: any): Promise<any[]> {
    ...
    try {
      const url = `https://site.api.espn.com/apis/site/v2/sports/hockey/mens-college-hockey/summary?event=${game.espn_game_id}`;
  ...
  private async parseSkaterStats(player: any, game: any, teamId: number): Promise<any> {
    try {
      ...
      // ESPN NCAA Hockey Skater stats order: G, A, P, +/-, PIM, PPG, SHG, SOG, TOI
  ...
  private async parseGoalieStats(player: any, game: any, teamId: number): Promise<any> {
    try {
      ...
      // ESPN NCAA Hockey Goalie stats order: MIN, GA, SA, SV, SV%, W-L-OT
      const values = player.stats;
  ...
  private async getOrCreatePlayer(player: any, teamId: number): Promise<number | null> {
    ...
    try {
      ...
      const result = await pgPool.query(
        ...
        RETURNING id`,
        [
          `ncaa_hockey_${player.athlete.id}`,
          'NCAA_HOCKEY',
...
if (require.main === module) {
  const collector = new NCAAHockeyStatsCollector();
