...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pgl_game_team 
  ON player_game_logs(game_id, team_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pgl_player_game 
  ON player_game_logs(player_id, game_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pgl_created 
  ON player_game_logs(created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pgl_game_date 
  ON player_game_logs(game_date);

-- Composite index for complex queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pgl_game_team_player 
  ON player_game_logs(game_id, team_id, player_id);

-- Fantasy points index for sorting
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pgl_game_fantasy 
  ON player_game_logs(game_id, fantasy_points DESC);

-- JSONB index for stats queries (this might take longer)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_pgl_stats_gin 
