# 🚨 IMPORTANT: Run These SQL Commands FIRST! 🚨

Before running ANY data collection scripts, you MUST create these tables in your Supabase database.
...
2. **Run Each SQL Block Below** (in order)

## 1️⃣ Create Advanced ML Tables (REQUIRED)
...
## 4️⃣ Verify Tables Were Created
...
1. **Run the test suite first:**
   ```bash
   npx tsx scripts/test-ml-data-collection.ts
   ```

2. **If all tests pass, proceed with:**
   ```bash
   # Backfill existing games
   npx tsx scripts/collectors/backfill-ml-data.ts
   
   # Collect historical seasons
   npx tsx scripts/collectors/comprehensive-ml-data-collector.ts --quick
...
- 2 update triggers active
- Ready for ML data collection!