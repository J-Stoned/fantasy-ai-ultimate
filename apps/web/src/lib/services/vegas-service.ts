...
export class VegasService extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    // Load current Vegas lines (with error handling)
    try {
      await this.loadVegasLines();
    } catch (error) {
      console.log('⚠️ Vegas lines table not available, using mock data');
  ...
  /**
   * Generate mock data when database tables not available
   */
  private generateMockData(): void {
    const mockGames = [
      { id: 'game1', sport: 'NFL', teams: ['KC', 'BUF'], spread: -3, total: 52 },
      { id: 'game2', sport: 'NFL', teams: ['DAL', 'GB'], spread: 2.5, total: 48 },
      { id: 'game3', sport: 'NFL', teams: ['TB', 'SF'], spread: -1, total: 45 },
    ];
    ...
    mockGames.forEach(game => {
      const line: VegasLine = {
        ...
        spread: game.spread,
        total: game.total,
        home_ml: game.spread > 0 ? 100 + (game.spread * 20) : -100 - (Math.abs(game.spread) * 20),
        away_ml: game.spread < 0 ? 100 + (Math.abs(game.spread) * 20) : -100 - (game.spread * 20),
        implied_home_score: game.total / 2 + (game.spread / 2),
        implied_away_score: game.total / 2 - (game.spread / 2),
        books: ['Mock'],
        last_updated: new Date()
      };

      this.vegasCache.set(game.id, line);
      const impact = this.calculateVegasImpact(line);
      this.impactCache.set(game.id, impact);
    });
  }
