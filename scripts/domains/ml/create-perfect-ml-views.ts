...
async function validateAllViews(): Promise<MLViewStats[]> {
  ...
  for (const viewName of viewsToCheck) {
    try {
      // Check record count - using identifier escaping
      const countResult = await pgPool.query(`SELECT COUNT(*) as count FROM "${viewName.replace(/"/g, '""')}"`);
      const recordCount = parseInt(countResult.rows[0].count);

      // Check sports included - using identifier escaping
      const sportsResult = await pgPool.query(`
        SELECT DISTINCT sport 
        FROM "${viewName.replace(/"/g, '""')}" 
        WHERE sport IS NOT NULL 
        ORDER BY sport
      `);
      const sportsIncluded = sportsResult.rows.map(row => row.sport);

      // Check date range - using identifier escaping
