#!/usr/bin/env tsx
/**
 * 🚀 MASS STATS COLLECTOR
 * 
 * Collects stats for ALL games with ESPN IDs at maximum speed
 */
...
const limit = pLimit(20); // 20 concurrent requests for SPEED!

class MassStatsCollector {
  private stats = {
  ...
  async collectAllStats() {
    console.log(chalk.bold.red('🚀 MASS STATS COLLECTOR - MAXIMUM SPEED!'));
    console.log(chalk.yellow('Collecting stats for ALL games...'));
    console.log(chalk.gray('='.repeat(60)));

    // Get all games that need stats - NO LIMIT!
    ...
    console.log(chalk.cyan(`Total games to check: ${allGames.length}`));

    // Check which games already have stats
    console.log(chalk.yellow('Checking existing stats...'));
    
    const { data: gamesWithStats } = await enhancedDb.getClient()
      .from('player_game_logs')
      .select('game_id');
    
    const hasStats = new Set(gamesWithStats?.map(g => g.game_id) || []);
    const gamesNeedingStats = allGames.filter(g => !hasStats.has(g.id));

    console.log(chalk.green(`Games already have stats: ${hasStats.size}`));
      ...
      limit(async () => {
        try {
          ...
          if (this.stats.errors < 10) {
            console.error(chalk.red(`  NBA error (${game.id}):`, error.message));
          }
        }
      })
    );
  ...
  private showResults() {
    ...
    if (totalGames > 0) {
      console.log(chalk.bold.green('\n✅ MASSIVE STATS COLLECTION SUCCESS!'));
      console.log(chalk.green(`🔥 Added ${totalPlayers} real player stats!`));
    }
  }
}

// Run the collector
const collector = new MassStatsCollector();
