...
const nextConfig = {
  ...
  // Security headers and caching strategy for 2025
  async headers() {
    return [
      ...
      // Real-time API routes - no caching
      {
        source: '/api/(predictions|ownership|ml)/:path*',
        headers: [
          {
            key: 'Cache-Control',
      ...
      // Default API routes - minimal caching
      {
        source: '/api/:path*',
        headers: [
          {
