...
export class InjuryService extends EventEmitter {
  ...
  private async loadInjuryData(): Promise<void> {
    const query = `
      ...
      WHERE pi.injury_status IS NOT NULL
  ...
  private async checkForUpdates(): Promise<void> {
    ...
    const query = `
      SELECT 
        pi.player_id,
        p.name as player_name,
        pi.injury_status as new_status,
        pi.last_updated
      FROM player_injuries pi
      JOIN players p ON pi.player_id = p.id
      WHERE pi.last_updated > $1`;
  ...
  async updatePlayerInjuryStatus(
    ...
    const query = `
      INSERT INTO player_injuries 
        (player_id, injury_status, injury_type, body_part, return_date, news, last_updated)
