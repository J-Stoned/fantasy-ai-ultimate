#!/usr/bin/env tsx
/**
 * 🏈 NCAA FOOTBALL STATS COLLECTOR - ULTRA OPTIMIZED
 * 
 * Collects detailed stats for all 2,129 NCAAF games
 * Similar to NFL but with college-specific stats
 */
...
const API_CONCURRENCY = 100; // 100 concurrent API calls for NCAAF
const DB_BATCH_SIZE = 1000; // Insert 1000 stats at once

export class NCAAFStatsCollector {
  ...
  constructor() {
    ...
    console.log(chalk.green.bold('\n🏈 NCAA FOOTBALL STATS COLLECTOR - 2,129 GAMES\n'));
  ...
  async collect() {
    try {
      // Cache all NCAAF players first
      await this.cacheNCAAFPlayers();
      
      // Get all NCAAF games needing stats
      ...
      console.log(chalk.cyan(`Found ${this.totalGames} NCAAF games needing stats\n`));
      ...
      console.log(chalk.green.bold(`\n✅ NCAAF STATS COLLECTION COMPLETE!`));
      ...
      console.error(chalk.red('❌ NCAAF stats collection failed:'), error);
    } finally {
      // Don't end pool - let master collector handle it
    }
  }
  
  private async cacheNCAAFPlayers() {
    console.log(chalk.cyan('📦 Caching NCAAF players...'));
    ...
    console.log(chalk.green(`✅ Cached ${players.rows.length} NCAAF players\n`));
  }
  ...
  private async collectGameStats(game: any): Promise<any[]> {
    ...
    try {
      const url = `https://site.api.espn.com/apis/site/v2/sports/football/college-football/summary?event=${game.espn_game_id}`;
...
if (require.main === module) {
  const collector = new NCAAFStatsCollector();
