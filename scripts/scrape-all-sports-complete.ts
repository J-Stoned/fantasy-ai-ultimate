#!/usr/bin/env tsx
/**
 * COMPLETE STATS SCRAPER - ALL SPORTS 2023-2024
 * Includes extraction functions for NBA, NFL, MLB, NHL, NCAAB, NCAAF
 */
...
console.log(chalk.bold.red(`🚀 COMPLETE STATS SCRAPER - ALL SPORTS!`))
...
async function scrapeAllSports() {
  ...
  for (const sport of Object.keys(SPORT_CONFIGS)) {
    console.log(chalk.gray(`Loading ${sport.toUpperCase()} games...`))
    const games = []
    offset = 0
  ...
  for (const [sport, games] of Object.entries(gamesBySport)) {
    ...
    for (let i = 0; i < games.length; i += batchSize) {
          ...
          try {
            ...
            if (response.status === 200 && response.data.boxscore) {
              ...
              if (stats.length > 0) {
                ...
                if (sportSuccessful % 20 === 0) {
                  console.log(chalk.green(`  ✅ ${sportSuccessful} games, ${sportStats.toLocaleString()} player stats`))
                }
              }
      ...
      if (totalProcessed % 200 === 0) {
        ...
        const remaining = (totalGames - totalProcessed) / rate
        console.log(chalk.cyan(
          `Overall: ${totalProcessed}/${totalGames} - ` +
    ...
    console.log(chalk.green(
      `${sport.toUpperCase()} complete: ${sportSuccessful} games, ${sportStats.toLocaleString()} player stats` +
          ...
          if (data.athletes && Array.isArray(data.athletes)) {
            data.athletes.forEach((athlete: any) => {
              ...
              if (data.name === 'passing' && athlete.stats.length >= 9) {
                ...
                const completions = athlete.stats[0].split('/')[0]
                const attempts = athlete.stats[0].split('/')[1]
                ...
                playerStats.stats = {
                  ...
                  sacks_taken: parseInt(athlete.stats[5]?.split('-')?.[0]) || 0,
                ...
                playerStats.stats = {
                  ...
                  rushing_long: parseInt(athlete.stats[4]) || 0
                }
                ...
                playerStats.stats = {
                  ...
                  receiving_long: parseInt(athlete.stats[4]) || 0,
                  targets: parseInt(athlete.stats[5]) || 0
                }
              } else if (data.name === 'defensive' && athlete.stats.length >= 6) {
                // TOT, SOLO, SACKS, TFL, PD, TD
                playerStats.stats = {
                  tackles_total: parseInt(athlete.stats[0]) || 0,
                  tackles_solo: parseInt(athlete.stats[1]) || 0,
                  sacks: parseFloat(athlete.stats[2]) || 0,
                  tackles_for_loss: parseFloat(athlete.stats[3]) || 0,
                  passes_defended: parseInt(athlete.stats[4]) || 0,
                  defensive_touchdowns: parseInt(athlete.stats[5]) || 0
                }
              }
        ...
        Object.values(teamData.statistics || {}).forEach((statGroup: any) => {
          if (statGroup.athletes && Array.isArray(statGroup.athletes)) {
            statGroup.athletes.forEach((athlete: any) => {
              ...
              if (statGroup.name === 'batting' && athlete.stats.length >= 9) {
                ...
                playerStats.stats = {
                  ...
                  batting_average: parseFloat(athlete.stats[6]) || 0,
                  on_base_percentage: parseFloat(athlete.stats[7]) || 0,
                  slugging_percentage: parseFloat(athlete.stats[8]) || 0
                }
              } else if (statGroup.name === 'pitching' && athlete.stats.length >= 9) {
                // IP, H, R, ER, BB, K, HR, ERA, PC-ST
                playerStats.stats = {
                  ...
                  era: parseFloat(athlete.stats[7]) || 0,
                  pitch_count: parseInt(athlete.stats[8]?.split('-')?.[0]) || 0
                }
              }
        ...
        Object.values(teamData.statistics || {}).forEach((statGroup: any) => {
          if (statGroup.athletes && Array.isArray(statGroup.athletes)) {
            statGroup.athletes.forEach((athlete: any) => {
              ...
              if (statGroup.name === 'skaters' && athlete.stats.length >= 8) {
              ...
              } else if (statGroup.name === 'goalies' && athlete.stats.length >= 6) {
                // SA, GA, SV, SV%, TOI, PIM
                const toi = athlete.stats[4] || '00:00'
                const minutes = parseInt(toi.split(':')[0]) || 0
                ...
                playerStats.stats = {
                  ...
                  penalty_minutes: parseInt(athlete.stats[5]) || 0
                }
              }
