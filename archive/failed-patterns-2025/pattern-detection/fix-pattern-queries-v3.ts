// Fixed pattern queries for your actual schema - VERSION 3
// Handles multiple status formats: 'Final', 'STATUS_FINAL', 'completed'
export const FIXED_PATTERN_QUERIES = {
  backToBackFade: `
    WITH team_games AS (
      ...
      FROM games g
      WHERE g.status IN ('Final', 'STATUS_FINAL', 'completed')
    )
    SELECT 
      tg.*,
    ...
    WHERE EXTRACT(EPOCH FROM (tg.start_time::timestamp - tg.prev_game_time::timestamp)) / 3600 < 30
  ...
  revengeGame: `
    WITH matchups AS (
      ...
      JOIN games g2 ON 
        ...
        AND g2.status IN ('Final', 'STATUS_FINAL', 'completed')
      WHERE g1.status IN ('Final', 'STATUS_FINAL', 'completed')
    )
    SELECT DISTINCT ON (id) 
      m.*,
      bl.home_line as home_spread,
      bl.home_odds as home_moneyline,
      bl.away_line as away_spread,
      bl.away_moneyline
    FROM matchups m
