-- 🚀 ADVANCED ML TABLES FOR 70%+ ACCURACY (FIXED FOR BIGINT IDs)
-- Fixed to match existing schema where player/team/game IDs are BIGINT
...
CREATE TABLE IF NOT EXISTS team_synergy_stats (
  ...
  player_ids BIGINT[] NOT NULL,  -- Array of player IDs in lineup
  sport TEXT NOT NULL,
...
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_advanced_metrics_modtime') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_synergy_modtime') THEN
...
LEFT JOIN information_schema.tables 
    ON information_schema.tables.table_name = required_tables.table_name
