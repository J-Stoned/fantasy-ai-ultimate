#!/usr/bin/env tsx
/**
 * UUID-FIXED PRODUCTION COLLECTOR
 * Properly handles UUID game_id requirements
 */
...
class UUIDFixedCollector {
  ...
  async run() {
    console.log('🚀 UUID-FIXED PRODUCTION COLLECTOR');
    console.log('=================================\n');
  ...
  async collectSleeperPlayers() {
    ...
    try {
      ...
      const activePlayers = players
        ...
        .slice(0, 500); // Process first 500
      
      console.log(`Processing ${activePlayers.length} active players...`);
      
      const chunks = this.chunkArray(activePlayers, 50);
  ...
  async collectESPNData() {
    ...
    for (const week of weeks) {
      ...
      try {
        ...
        // Process first 3 games per week
        for (const game of completedGames.slice(0, 3)) {
  ...
  async processESPNGame(game: any) {
    try {
      // Store game and get its UUID
      const { data: gameRecord } = await supabase.from('games').upsert({
      ...
      if (summary.data.boxscore?.players) {
        for (const teamData of summary.data.boxscore.players) {
          await this.processTeamStats(teamData, game.id, gameRecord.id);
        }
      }
  ...
  async processTeamStats(teamData: any, espnGameId: string, gameUuid: string) {
    ...
    for (const statCategory of teamData.statistics || []) {
      if (statCategory.athletes?.length > 0) {
        ...
        for (const playerData of statCategory.athletes.slice(0, 3)) {
  ...
  async storePlayerWithStats(playerData: any, espnGameId: string, gameUuid: string, category: string, teamName: string, teamAbbr: string) {
    try {
      ...
      if (player) {
        ...
        if (fantasyPoints > 0 || this.hasStats(stats)) {
          const { error: logError } = await supabase
            ...
            .insert({
              player_id: player.id,
              game_id: gameUuid, // Use the UUID from games table
              game_date: new Date(),
              stats: stats,
  ...
  async processNBAGame(game: any) {
    try {
      ...
      if (!gameRecord) {
        console.error('Failed to create NBA game record');
        return;
      }
      ...
      if (summary.data.boxscore?.players) {
        for (const teamData of summary.data.boxscore.players) {
          ...
          if (scoringCategory?.athletes) {
            for (const player of scoringCategory.athletes.slice(0, 2)) {
              await this.storeNBAPlayer(player, game.id, gameRecord.id, teamData.team);
            }
          }
        }
      }
  ...
  async storeNBAPlayer(playerData: any, espnGameId: string, gameUuid: string, team: any) {
    try {
      const athlete = playerData.athlete;
  ...
  async checkProgress() {
    ...
    console.log('\n🎯 TARGET FOR 76.4% ACCURACY:');
    console.log(`Current game logs: ${totalLogs}`);
    console.log(`Target game logs: ~980,000 (20 per game)`);
    console.log(`Progress: ${((totalLogs || 0) / 980000 * 100).toFixed(2)}%`);
  }
}

// Run it!
new UUIDFixedCollector().run();