...
export class ARStatsOverlay {
  ...
  private async matchPlayer(
    ...
    try {
      // Call API to match player
      const response = await fetch('/api/ar/player-match', {
        method: 'POST',
        headers: {
      ...
      console.error('Player match API error:', error);
    }
  ...
  async getPlayerStats(playerId: string): Promise<ARStats | null> {
    ...
    try {
      const response = await fetch(`/api/ar/player-stats?playerId=${playerId}`);
      const data = await response.json();
