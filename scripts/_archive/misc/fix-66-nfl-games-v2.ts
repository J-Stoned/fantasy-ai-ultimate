#!/usr/bin/env tsx
/**
 * Fix the 66 NFL games without stats - targeted collector V2
 * Uses correct schema with stats JSON column
 */
...
class NFLGameFixerV2 {
  ...
  async processGame(gameId: number, externalId: string, gameDate: string) {
    try {
      ...
      if (error.response?.status === 400) {
        ...
        if (externalId.startsWith('espn_')) {
          ...
          return this.processGame(gameId, cleanId, gameDate);
        }
      }
  ...
  calculateFantasyPoints(stats: any): number {
    ...
    points += (stats.receiving_receptions || 0) * 0.5; // PPR
  ...
  async run() {
    console.log(chalk.bold.cyan('\n🔧 NFL GAME FIXER V2 - TARGETING 66 SPECIFIC GAMES\n'));
    ...
    for (let i = 0; i < fixableGames.length; i++) {
      ...
      await this.processGame(game.gameId, game.externalId, game.date);
    ...
    const totalCount = all2024Games?.length || 432;
    const newCoverage = ((gamesWithStats / totalCount) * 100).toFixed(1);
    
    console.log(chalk.bold.green(`\n✨ NEW COVERAGE: ${newCoverage}%`));
    console.log(`   Was: 84.7%`);
    console.log(`   Now: ${newCoverage}%`);
...
// Run the fixer
const fixer = new NFLGameFixerV2();
