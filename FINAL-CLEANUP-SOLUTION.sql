-- 🚨 FINAL CLEANUP SOLUTION - RUN THIS IN SUPABASE SQL EDITOR
...
BEGIN
    ...
    DELETE FROM team_stats WHERE game_id = ANY(game_ids);
    DELETE FROM game_odds WHERE game_id = ANY(game_ids);
    DELETE FROM game_weather WHERE game_id = ANY(game_ids);
...
BEGIN
    ...
    LOOP
        ...
        -- Log progress
        RAISE NOTICE 'Batch %: Deleted % games and % stats', 
            batch_count, batch_result.deleted_count, batch_result.stats_deleted;
    END LOOP;
...
BEGIN
    ...
    RAISE NOTICE 'Step 1: Deleting fake games...';
    ...
    RAISE NOTICE 'Remaining players: %', (SELECT COUNT(*) FROM players);
    RAISE NOTICE 'Remaining games: %', (SELECT COUNT(*) FROM games);
