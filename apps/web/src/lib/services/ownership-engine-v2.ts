...
export class OwnershipEngineV2 extends EventEmitter {
  ...
  constructor(
    ...
    // Temporarily disable historical loading to isolate the issue
    // this.loadHistoricalOwnership();
    console.log('🔍 [ELITE DEBUG] OwnershipEngineV2 constructor completed');
  }
  ...
  private async loadHistoricalOwnership(): Promise<void> {
    try {
      ...
      console.log('No historical ownership table available yet');
    }
  }
  ...
  async projectSlateOwnership(
    ...
    console.log('🔍 [ELITE DEBUG] Starting projectSlateOwnership...');
    
    // Skip cache to isolate issue
    ...
    console.log(`🧠 [ELITE DEBUG] Projecting ${sport} ${slate} ownership for ${gameDate.toDateString()}`);
    
    console.log('🔍 [ELITE DEBUG] About to call getSlatePlayers...');
    // Get all players in slate
    const players = await this.getSlatePlayers(sport, slate, gameDate);
    console.log(`🔍 [ELITE DEBUG] getSlatePlayers returned ${players.length} players`);
    ...
    // Cache results (disabled for now)
  ...
  private async getSlatePlayers(sport: string, slate: string, gameDate: Date): Promise<any[]> {
    // First, let's get players directly from the database since the complex joins may not exist
    const query = `
      SELECT 
        p.id::text as id,
        p.name,
        p.position,
        p.team,
        8000 as salary,  -- Default salary for testing
        15.0 as projected_points,  -- Default projection
        12.0 as floor,
        20.0 as ceiling,
        NOW() as game_time,
        true as is_home,
    ...
    const result = await this.pool.query(query, [sport.toUpperCase()]);
    return result.rows;
  }
  ...
  private async calculateOwnershipFactors(player: any, gameDate: Date): Promise<OwnershipFactors> {
    ...
    // Get from ownership_factors table (if it exists)
    let factorsQuery;
    try {
      factorsQuery = await this.pool.query(`
        SELECT * FROM ownership_factors
        WHERE player_id = $1::text AND game_date = $2
      `, [player.id, gameDate]);
    } catch (error) {
      // Table may not exist, use empty result
      factorsQuery = { rows: [] };
    }
  ...
  private async getInjuryOpportunity(player: any): Promise<number> {
    try {
      const result = await this.pool.query(`
        ...
        JOIN player_injuries pi ON p2.id::text = pi.player_id
        WHERE p1.id::text = $1::text
      ...
      // Table may not exist or depth_rank column missing
      return 0.0;
    }
  }
  ...
  private async getPositionScarcity(player: any, gameDate: Date): Promise<number> {
    try {
      const result = await this.pool.query(`
        ...
        FROM players p
        JOIN player_salaries ps ON p.id::text = ps.player_id
        JOIN player_projections pp ON p.id::text = pp.player_id
        WHERE p.position = $1
          AND ps.game_date = $2
      ...
      // Tables may not exist, return default scarcity based on position
      const positionCounts: Record<string, number> = {
        'QB': 15, 'RB': 25, 'WR': 35, 'TE': 12, 'K': 20, 'DST': 16
      };
      const count = positionCounts[player.position] || 20;
      return 1 - Math.min(count / 30, 1);
    }
  }
