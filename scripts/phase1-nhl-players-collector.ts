#!/usr/bin/env tsx
/**
 * 🏒 PHASE 1: NHL PLAYERS COLLECTOR
 * 
 * Collects all NHL players for 2023-2025 seasons with standardized data
...
async function collectNHLPlayers(season: number) {
  console.log(chalk.blue.bold(`\n🏒 COLLECTING NHL PLAYERS FOR ${season} SEASON\n`));
  ...
  try {
    ...
    // Get all NHL teams from database
    ...
    if (!teams || teams.length === 0) {
      throw new Error('No NHL teams found in database');
    }
    
    console.log(chalk.white(`Found ${teams.length} NHL teams`));
    
    // ESPN team ID mapping for NHL
    ...
    console.error(chalk.red('❌ Error collecting NHL players:'), error);
    throw error;
  }
}

async function collectAllSeasons() {
  console.log(chalk.cyan.bold('\n🏒 NHL PLAYERS COLLECTION - PHASE 1\n'));
  ...
  for (const season of seasons) {
    try {
      ...
      if (season === 2024) { // Only collect current roster once
        const result = await collectNHLPlayers(season);
        results.push(result);
      } else {
  ...
  console.log(chalk.green(`\n✅ Total NHL players in database: ${count || 0}`));
  console.log(chalk.green('✅ Phase 1 NHL collection complete!'));
}
