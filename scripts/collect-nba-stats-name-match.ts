#!/usr/bin/env tsx
/**
 * 🏀 NBA STATS COLLECTOR - NAME MATCHING VERSION
 * Uses player name matching since external_ids don't match
 */
...
console.log(chalk.bold.yellow('🏀 NBA STATS COLLECTOR - NAME MATCHING VERSION\n'));
    ...
    for (const team of response.data.boxscore.teams) {
      ...
      if (team.statistics && team.statistics[0] && team.statistics[0].athletes) {
        for (const athlete of team.statistics[0].athletes) {
          ...
          // Get the correct team_id based on home/away
          const playerTeamId = isHome ? game.home_team_id : game.away_team_id;
...
async function collectNBAStats() {
  console.log('🚀 STARTING NBA STATS COLLECTION (NAME MATCHING)');
